...
if __name__ == '__main__':
    ...
    for mode in (5, 4, 3, 2, 1):
        for bitrate in range(600, 2100, 200):
                start = time.time()
                ...
                av1an.encoder = 'aom'
                av1an.encode_pass = 2
                av1an.encoding_params = f' --threads=8 --tune=vmaf --cpu-used={mode}  --end-usage=cbr --target-bitrate={bitrate}'
                av1an.output_file = f'film_av1'
                av1an.logging = f'a'
                av1an.main()
                ...
                # Vmaf_part
                cmd = f'ffmpeg -r 24 -hide_banner -loglevel error -an -i film.mp4 -i film_av1.mkv -filter_complex "[1:v][0:v]libvmaf=model_path=vmaf_v0.6.1.pkl:log_path=vmaf.log" -f null -'
                ...
                print(dt)
                with open('data_aomtune.txt', 'a') as f:
                    f.write(str(dt))
                    f.write(',')
