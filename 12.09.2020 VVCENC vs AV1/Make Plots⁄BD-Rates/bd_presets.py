...
with open(sys.argv[1]) as f:
    ...
    if data[0][0] == 'vvc':
        for i, x in enumerate(data):
            data[i][2] = vvc_sort(data[i][2])

rn = sorted(list({ x[2] for x in data }))

# rn = sorted(rn, key=vvc_sort)
print(rn)
...
for r in rn:
    ...
    for i in [x for x in rn if x < ii]:

        if data[0][0] == 'vvc':
            print('Bd rate for speeds: ', f'{vvc_back(ii).upper()} -> {vvc_back(i).upper()}', '\n')
        else:
            print('Bd rate for speeds: ', f'{ii} -> {i}', '\n')

        #Vmaf
        vmaf0 = [(x[4],x[5]) for x in data if x[2] == i ]
        vmaf1 = [(x[4],x[5]) for x in data if x[2] == ii ]
        ...
        time0 = [x[1] for x in data if x[2] == i ]
        time1 = [x[1] for x in data if x[2] == ii ]
