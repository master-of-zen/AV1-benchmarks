...
class Av1an:
    ...
    def arg_parsing(self):
        ...
        parser.add_argument('--output_file', '-o', type=str, default=self.output_file, help='Specify output file')
...
if __name__ == '__main__':
    ...
    output = []
    """
    for mode in reversed(range(4, 9)):
        for bitrate in range(1000, 2200, 200):
            for enc_pass in (1, 2):
                start = time.time()

                # Encoding
                av1an = Av1an()
                av1an.encoder = 'svt_av1'
                av1an.encode_pass = enc_pass
                av1an.encoding_params = f' -fps-num 24000 -fps-denom 1001 -rc 1 -tbr {bitrate} -enc-mode {mode} '
                av1an.output_file = f'svt{mode}_{bitrate}'
                av1an.logging = f'{bitrate}'
                av1an.main()
                os.popen('stty sane', 'r')
                end = round(time.time()-start, 1)

                # Vmaf_part
                cmd = f'ffmpeg -r 24 -hide_banner -loglevel error -an -i film.mkv  -i svt{mode}_{bitrate}.mkv -filter_complex ' \
                    f'"[1:v][0:v]libvmaf=model_path=vmaf_v0.6.1.pkl:log_path=vmaf_svt{mode}_{bitrate}.log" -f null -'
                temp = (subprocess.check_output(cmd, shell=True))
                temp = str(temp).split('=')[1].strip()[:-4]
                print()
                try:
                    dt = (mode, bitrate, float(temp), end, av1an.encode_pass)
                except Exception:
                    dt = (mode, bitrate, 0, end, av1an.encode_pass)

                print(dt)
                with open('data.txt', 'a') as f:
                    f.write(str(dt))
                    f.write(',')
                output.append((mode, bitrate, temp, end, av1an.encode_pass))
                os.popen('stty sane', 'r')
    """
    for mode in (8, 7, 6, 5, 4, 3, 2, 1):
        for bitrate in range(800, 2500, 200):
            start = time.time()
            ...
            av1an.encoder = 'svt_av1'
            av1an.encode_pass = 1
            av1an.encoding_params = f' -fps-num 24000 -fps-denom 1001 -w 1920 -h 800 -enc-mode {mode} -rc 1 -tbr {bitrate}'
            av1an.output_file = f'svt'
            av1an.logging = f'b'
            av1an.main()
            os.popen('stty sane', 'r')
            end = round(time.time()-start, 1)
            ...
            # Vmaf_part
            cmd = f'ffmpeg -r 24 -hide_banner -loglevel error -an -i film.mkv  -i svt.mkv -filter_complex ' \
            f'"[1:v][0:v]libvmaf=model_path=vmaf_v0.6.1.pkl:log_path=vmaf_svt.log" -f null -'
            temp = (subprocess.check_output(cmd, shell=True))
            temp = str(temp).split('=')[1].strip()[:-4]
            print()
            try:
                dt = (mode, bitrate, float(temp), end, )
            except Exception:
                dt = (mode, bitrate, 0, end, )
