...
if __name__ == '__main__':
    ...
    for mode in reversed(range(1, 11)):
        for bitrate in (150, 140, 130, 120, 110, 100, 90):
                start = time.time()
                ...
                av1an.encoder = 'rav1e'
                av1an.encode_pass = 1
                av1an.encoding_params = f' --tiles 4 --speed {mode} --tile-rows 1 --tile-cols 4'
                av1an.output_file = f'{mode}_{bitrate}_film_av1'
                av1an.logging = f'b'
                av1an.main()
                ...
                # Vmaf_part
                cmd = f'ffmpeg -r 24 -hide_banner -loglevel error -an -i film.mp4 -i {mode}_{bitrate}film_av1.mkv -filter_complex ' \
