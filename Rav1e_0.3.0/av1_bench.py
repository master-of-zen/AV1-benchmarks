...
if __name__ == '__main__':
    ...
    start = time.time()
    output = []
    for mode in reversed(range(0, 11)):
        for bitrate in range(800, 2500, 200):
            ...
            av1an = Av1an()
            av1an.encoder = 'rav1e'
            av1an.encoding_params = f' --tile-cols 4 --tile-rows 1 --speed {mode}  --bitrate {bitrate} '
            av1an.output_file = f'rav1e'
            av1an.logging = f'b'
            av1an.main()
            ...
            # Vmaf_part
            cmd = f'ffmpeg -hide_banner -loglevel error -r 24 -an -i film.mkv  -i rav1e.mkv -filter_complex ' \
                  f'"[1:v][0:v]libvmaf=model_path=vmaf_v0.6.1.pkl:log_path=vmaf_rav1e{mode}_{bitrate}.log" -f null -'
            ...
            with open('data.txt', 'a') as f:
                f.write(str(dt))
                f.write(',')
            output.append((mode, bitrate, temp, end))
            os.popen('stty sane', 'r')
