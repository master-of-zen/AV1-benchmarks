...
if __name__ == '__main__':
    ...
    for q in (10,15,20,25,30,35):
        ...
        av1an = Av1an()
        av1an.encoder = 'aom'
        av1an.encoding_params = f' --threads=8 --cpu-used=3  --end-usage=q --cq-level={q} '
        av1an.output_file = f'{q}'
        av1an.main()
        os.popen('stty sane', 'r')
        end = round(time.time()-start, 1)

        # Vmaf_part
        cmd = f'ffmpeg -hide_banner -loglevel error -r 24000/1001 -i crow.mkv -r 24000/1001 -i q.mkv -filter_complex "[1:v][0:v]libvmaf=model_path=vmaf_b_v0.6.3/vmaf_b_v0.6.3.pkl:pool=harmonic_mean:enable_conf_interval=1" -f null - '
        temp = (subprocess.check_output(cmd, shell=True))
        temp = str(temp).split('=')[1].strip()[:-4]
        print()
        try:
            dt = ('q', q, float(temp), end)
        except Exception:
            dt = ('q', q, 0, end)
        print(dt)
        with open('crow.txt', 'a') as f:
            f.write(str(dt))
            f.write(',')
        output.append(('q', q, 0, end))
        os.popen('stty sane', 'r')
