...
if __name__ == '__main__':
    ...
    output = []
    for mode in ( 8, 7 ,6, 5):
        for bitrate in (10,30,50,70,90,110,130,150):
                start = time.time()
                ...
                av1an.encoder = 'rav1e'
                av1an.encode_pass = 1
                av1an.encoding_params = f' --quantizer {bitrate} -s {mode} --tile-rows 1 --tile-cols 4'
                av1an.output_file = f'rav1e{mode}_{bitrate}'
                av1an.logging = f'rav1e'
                av1an.main()
                os.popen('stty sane', 'r')
                end = round(time.time()-start,1)

                # Vmaf_part
                cmd = f'ffmpeg -r 24 -hide_banner -loglevel error -an -i film.mp4  -i rav1e{mode}_{bitrate}.mkv -filter_complex ' \
                      f'"[1:v][0:v]libvmaf=model_path=vmaf_v0.6.1.pkl:log_path=vmaf.log" -f null -'

                temp = (subprocess.check_output(cmd, shell=True))
                temp = str(temp).split('=')[1].strip()[:-4]
                print()
                try:
                    dt = (mode, bitrate, float(temp), end * 2, )
                except Exception:
                    dt = (mode, bitrate, 0, end * 2, )

                print(dt)
                with open('data_rav1e.txt', 'a') as f:
                    f.write(str(dt))
                    f.write(',')
                os.popen('stty sane', 'r')
